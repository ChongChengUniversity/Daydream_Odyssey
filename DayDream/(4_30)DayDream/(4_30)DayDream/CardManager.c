...
void InitCardTypes() {
    ...
    for (int i = 0; i < 10; ++i) cardTypes[index++] = TYPE_ITEM;
    for (int i = 0; i < 5; ++i) cardTypes[index++] = TYPE_ENEMY;
    while (index < TOTAL_CARDS) cardTypes[index++] = TYPE_EMPTY;
...
void InitCards() {
    ...
    for (int row = 0; row < ROWS; ++row) {
        for (int col = 0; col < COLS; ++col) {
            ...
            case TYPE_DOOR:  cards[card_index] = CreateDoorCard(x, y); break;
            case TYPE_ITEM:  cards[card_index] = CreateItemCard(x, y); break;
            case TYPE_ENEMY: cards[card_index] = CreateEnemyCard(x, y); break;
            case TYPE_EMPTY: cards[card_index] = CreateEmptyCard(x, y); break;
            }
            card_index++;
        }
    }
    ...
    for (int i = 0; i < TOTAL_CARDS; ++i) {
        if (cards[i] && CheckCollisionPointRec(mousePos, cards[i]->bounds)) {
            if (!cards[i]->isRevealed) {
                ...
                if (cards[i]->onInteract) { // 有設定互動行為才執行
